...
function handleDeleteSetClick(incomingSet) {
    ...
    setCurrentSets(currentSets.filter((set) => set.id !== incomingSet.id))
}

const emptySet = {weight: 0, reps: 0}

    return (
        <article className="bg-tiny-orange p-4 flex flex-col place-items-center rounded-lg justify-between">
        ...
        <section className="flex flex-col gap-2">
            {currentSets.length > 0 && 
                currentSets.map((set, index) => {
                    return (
                        <SetBar key={index + "set"} set={set} handleDeleteSetClick={handleDeleteSetClick} />
                    )
                })
            }
        </section>
